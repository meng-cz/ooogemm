// Compatibility wrapper for legacy users of the former static top name.
// The implementation is the current blocksel + new_static_uopparse design;
// no legacy parser is instantiated here.
...
module top_static #(
    ...
    top_new_static #(
        ...
        .LANE_IDX_WIDTH(LANE_IDX_WIDTH), .ABUF_IDX_WIDTH(ABUF_IDX_WIDTH),
        .BBUF_IDX_WIDTH(BBUF_IDX_WIDTH), .PACC_IDX_WIDTH(PACC_IDX_WIDTH),
        .ROW8_WIDTH(ROW8_WIDTH), .LOAD_BUS_ID_WIDTH(LOAD_BUS_ID_WIDTH),
        .ROW32_WIDTH(ROW32_WIDTH), .LOAD_ROWS_WIDTH(LOAD_ROWS_WIDTH),
        .STORE_MEM_DATA_WIDTH(STORE_MEM_DATA_WIDTH),
        .GEMM_INSTID_WIDTH(GEMM_INSTID_WIDTH),
        .COUNT_WIDTH(OUTSTANDING_CNT_WIDTH)
    ) impl (
        .clk(clk), .rst_n(rst_n),
        ...
        .cmd_done_valid_o(unused_cmd_done)
    );
endmodule
