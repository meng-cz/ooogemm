...
// Fixed synthesis wrapper for the dynamic dynamic_L4_W32_AB12_16_ACC24_32 experiment configuration.
module dynamic_L4_W32_AB12_16_ACC24_32 #(
    parameter int SA_WIDTH = 32,
    parameter int SUBTILE_M = 32,
    parameter int SUBTILE_N = 32,
    parameter int SUBTILE_K = 32,
    parameter int LANE_NUM = 4,
    parameter int ABUF_SIZE = 16,
    parameter int BBUF_SIZE = 16,
    parameter int PACC_NUM = 32,
    parameter int ABUF_LOGIC_SIZE = 12,
    parameter int BBUF_LOGIC_SIZE = 12,
    parameter int PACC_LOGIC_SIZE = 24,
    parameter int ADDR_WIDTH = 32,
    parameter int DIM_WIDTH = 16,
    parameter int STORE_ROWS_PER_CYCLE = 2,
    ...
    top_dynamic #(
        ...
        .ABUF_LOGIC_SIZE(ABUF_LOGIC_SIZE), .BBUF_LOGIC_SIZE(BBUF_LOGIC_SIZE),
        .PACC_LOGIC_SIZE(PACC_LOGIC_SIZE), .GEMM_TRACK_DEPTH(64), .GEMM_INSTID_WIDTH(GEMM_INSTID_WIDTH)
    ) impl (
        .clk(clk), .rst_n(rst_n),
