...
MEM_DATA_WIDTH="${MEM_DATA_WIDTH:-$((SA_WIDTH * 32 * ROWS_PER_CYCLE))}"
...
verilator \
  ...
  -CFLAGS "-DSA_WIDTH_TEST=${SA_WIDTH} -DPACC_NUM_TEST=${PACC_NUM} -DPACC_IDX_WIDTH_TEST=${PACC_IDX_WIDTH} -DROWS_PER_CYCLE_TEST=${ROWS_PER_CYCLE} -DMEM_DATA_WIDTH_TEST=${MEM_DATA_WIDTH}" \
