...
for LANE_NUM in ${LANE_NUMS}; do
  ...
  verilator \
    ...
    -CFLAGS "-DSA_WIDTH_TEST=${SA_WIDTH} -DSUBTILE_K_TEST=${SUBTILE_K} -DLANE_NUM_TEST=${LANE_NUM} -DABUF_SIZE_TEST=${ABUF_SIZE} -DBBUF_SIZE_TEST=${BBUF_SIZE} -DPACC_NUM_TEST=${PACC_NUM} -DPACC_EXP_WIDTH_TEST=${PACC_EXP_WIDTH} -DPACC_SIG_WIDTH_TEST=${PACC_SIG_WIDTH} -DSTORE_ROW_WRITE_BEATS_TEST=${STORE_ROW_WRITE_BEATS} -DTOP_STATIC_BIG_TEST=${BIG}" \
