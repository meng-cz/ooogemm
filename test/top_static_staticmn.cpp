#include "Vtop_static_staticmn.h"
#include "verilated.h"
...
#endif
#ifndef TOP_STATIC_STATICMN_PERF_TEST
#define TOP_STATIC_STATICMN_PERF_TEST 0
#endif
#ifndef PERF_M_TEST
...
constexpr int choose_block_m(int remaining_m, int remaining_n,
    ...
    int best_m = 1, best_area = 0;
    int best_balance = 1 << 30;
    for (int cm = 1; cm <= a_group; ++cm) {
        const int cn = std::min(b_group, block_cap / cm);
        if (cn < 1) continue;
    ...
    return std::min(remaining_m, best_m);
}

constexpr int choose_block_n(int remaining_m, int remaining_n,
                             int a_group, int b_group, int block_cap) {
    int best_m = 1, best_n = 1, best_area = 0;
    int best_balance = 1 << 30;
    for (int cm = 1; cm <= a_group; ++cm) {
        const int cn = std::min(b_group, block_cap / cm);
        if (cn < 1) continue;
    ...
    return std::min(remaining_n, best_n);
}
...
#if TOP_STATIC_STATICMN_PERF_TEST
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // TM*TN=1 is below the merge capacity of the configured logical
        // resources, so eight independent batch tiles share each K-wave
        // block instead of becoming eight separate MNK blocks.
        run_case("merged_batch_8", 2, 2, 16, 8, 5, 128,
                 16 / STORE_ROWS_PER_CYCLE_TEST, 16.0f);
        // With ABUF/BBUF=16 and PACC=32, the logical double-buffered
        // resources are 8/8/16, so the static selector chooses a 4x4 block.
        // This case spans four tiles in both M and N and exercises that
        // compile-time maximum-area selection rather than tile truncation.
        run_case("max_area_block", 8, 8, 8, 1, 9, 32,
                 32 / STORE_ROWS_PER_CYCLE_TEST, 8.0f);
#endif
#endif
        std::cout << "top_static_staticmn tests passed\n";
    } catch (const std::exception& e) {
        std::cerr << "top_static_staticmn test failed: " << e.what() << "\n";
        return 1;
    }
    return 0;
}
