...
verilator \
  ...
  -CFLAGS "-O2 -g0 -DSA_WIDTH_TEST=${SA_WIDTH} -DSUBTILE_K_TEST=${SUBTILE_K} -DABUF_SIZE_TEST=${ABUF_SIZE} -DBBUF_SIZE_TEST=${BBUF_SIZE} -DPACC_NUM_TEST=${PACC_NUM} -DSTORE_ROWS_PER_CYCLE_TEST=${STORE_ROWS_PER_CYCLE} -DLOAD_DATA_WIDTH_TEST=${LOAD_DATA_WIDTH} -DTOP_NEW_STATIC_PERF_TEST=${PERF_TEST} -DPERF_M_TEST=${PERF_M} -DPERF_N_TEST=${PERF_N} -DPERF_K_TEST=${PERF_K} -DPERF_BATCH_TEST=${PERF_BATCH}" \
