...
constexpr int kWritesPerOutputTile = kSubtileM / kStoreRowsPerCycle;
...
public:
    explicit StaticLab(const Options& opt) : opt_(opt) {
        ...
        output_tiles_per_cmd_ =
            static_cast<uint64_t>(ceil_div(opt_.m, kSubtileM)) *
            static_cast<uint64_t>(ceil_div(opt_.n, kSubtileN));
...
private:
    ...
    uint64_t default_max_cycles() const {
        const uint64_t tm = static_cast<uint64_t>(ceil_div(opt_.m, kSubtileM));
        const uint64_t tn = static_cast<uint64_t>(ceil_div(opt_.n, kSubtileN));
