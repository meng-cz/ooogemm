...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run static GEMM axis-sweep experiments with a Python worker pool."
    )
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            ...
            "1:128:32:32:128:1 4:64:32:32:128:2 16:32:32:32:128:4",
        ),
    )
    parser.add_argument("--data-root", type=Path, default=Path(os.environ.get("DATA_ROOT", ROOT_DIR / "data/rect_static")))
    parser.add_argument("--log-dir", type=Path, default=Path(os.environ.get("LOG_DIR", ROOT_DIR / "data/rect_static/log")))
...
def build_all_tasks(args: argparse.Namespace) -> list[Task]:
    ...
    hardware = parse_hardware_configs(
        ...
        [
            Hardware(1, 128, 32, 32, 128, 1),
            Hardware(4, 64, 32, 32, 128, 2),
            Hardware(16, 32, 32, 32, 128, 4),
        ],
    )
...
def run_task(
    ...
    with print_lock:
        print(
            f"static_lab: task_start order={index}/{total} {fields} "
    ...
    with print_lock:
        ...
        print(
            f"static_lab: task_{status} order={index}/{total} {fields} "
            f"{task_label(task)} "
...
def main() -> int:
    ...
    if args.jobs <= 0:
        print("static_lab: --jobs must be positive", file=sys.stderr)
        return 2
    ...
    print(
        f"static_lab: generated={len(all_tasks)} pending={len(pending)} "
    ...
    try:
        ...
        print(f"static_lab: interrupted; terminating {len(children)} child processes", file=sys.stderr)
    ...
    if failures:
        print(f"static_lab: failures={len(failures)}", file=sys.stderr)
        for task, rc in failures:
            print(f"  rc={rc} log={task.log_file} output={task.out_file}", file=sys.stderr)
        return 1

    print("static_lab: all pending tasks completed", flush=True)
    return 0
