...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run static_staticmn GEMM axis-sweep experiments with a Python worker pool."
    )
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            "HARDWARE_CONFIGS",
            "1:64:32:32:128:1 4:32:32:32:128:2 16:16:32:32:128:4",
        ),
    )
    parser.add_argument("--data-root", type=Path, default=Path(os.environ.get("DATA_ROOT", ROOT_DIR / "data/static_staticmn")))
    parser.add_argument("--log-dir", type=Path, default=Path(os.environ.get("LOG_DIR", ROOT_DIR / "data/static_staticmn/log")))
...
def prebuild_hardware(args: argparse.Namespace, tasks: list[Task], print_lock: threading.Lock) -> bool:
    ...
    for hw in hardware:
        ...
        with print_lock:
            print(
                f"static_staticmn_lab: prebuild {hardware_tag(hw)} "
                f"log={log_file}",
                flush=True,
            )
        ...
        if rc != 0:
            with print_lock:
                print(
                    f"static_staticmn_lab: prebuild failed {hardware_tag(hw)} "
...
def run_task(
    ...
    with print_lock:
        print(
            f"static_staticmn_lab: task_start order={index}/{total} {fields} "
    ...
    with print_lock:
        ...
        print(
            f"static_staticmn_lab: task_{status} order={index}/{total} {fields} "
            f"{task_label(task)} "
...
def main() -> int:
    ...
    if args.jobs <= 0:
        print("static_staticmn_lab: --jobs must be positive", file=sys.stderr)
        return 2
    ...
    print(
        f"static_staticmn_lab: generated={len(all_tasks)} pending={len(pending)} "
    ...
    for i in range(worker_count):
        thread = threading.Thread(target=worker, name=f"static-lab-worker-{i}", daemon=False)
        thread.start()
    ...
    try:
        ...
        print(f"static_staticmn_lab: interrupted; terminating {len(children)} child processes", file=sys.stderr)
    ...
    if failures:
        print(f"static_staticmn_lab: failures={len(failures)}", file=sys.stderr)
        for task, rc in failures:
            print(f"  rc={rc} log={task.log_file} output={task.out_file}", file=sys.stderr)
        return 1

    print("static_staticmn_lab: all pending tasks completed", flush=True)
    return 0
