...
public:
    ...
    void run() {
        ...
        run_until_done();
        write_result();
        std::cout << "static_staticmn_lab: passed"
                  << " L=" << kLaneNum
                  << " W=" << kSaWidth
...
private:
    ...
    void record_store(uint32_t wr_addr) {
        ...
        if (seen == expected_writes_per_cmd_) {
            ...
            std::cout << "static_staticmn_lab: progress completed=" << completed_cmds_
                      << "/" << opt_.count
                      << " cmd=" << cmd_idx
                      << " cycle=" << cycle_
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cerr << "static_staticmn_lab failed: " << e.what() << "\n";
        return 1;
    }
}
